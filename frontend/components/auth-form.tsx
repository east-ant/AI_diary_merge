  ...
  try {
    if (isLogin) {
      ...
      if (data.success) {
        ...
        toast({
          title: "로그인 실패",
          description: data.msg || "아이디 혹은 비밀번호를 확인하세요.",
          variant: "destructive",
        })
      }
      ...
      // ✅ 여기서 회원가입 데이터 구조 변경
      const randomId = Math.random().toString(36).substring(2, 10) // 랜덤 8자리 키
      const username = email.split("@")[0] // 이메일 @ 앞부분 추출
      const createdAt = new Date().toISOString() // 현재 시간 (ISO 포맷)
      ...
      if (data.success) {
        ...
        toast({
          title: "회원가입 실패",
          description: data.msg || "서버 오류가 발생했습니다.",
          variant: "destructive",
        })
      }
    }
